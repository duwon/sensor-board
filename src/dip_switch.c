...
int Get_Switch(uint8_t *raw, struct dip_bits *parsed)
{
    ...
    if (r) { LOG_ERR("TCA CFG write: %d", r); goto Err; }
    r = tca_wr(REG_POLARITY, 0x00);
    if (r) { LOG_ERR("TCA POL write: %d", r); goto Err; }

    /* 3) 입력 2회 읽어 간단 디바운싱 */
    uint8_t v1=0, v2=0;
    r = tca_rd(REG_INPUT, &v1);             
	if (r) { LOG_ERR("TCA IN rd(1): %d", r); goto Err; }
    k_busy_wait(200); /* 0.2ms 대기 */
    r = tca_rd(REG_INPUT, &v2);             
	if (r) { LOG_ERR("TCA IN rd(2): %d", r); goto Err; }
